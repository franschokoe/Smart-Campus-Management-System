...
public class LibrarySystem {
    ...
    public void addBook(Scanner scanner) {
        ...
        String isbn = readIsbn(scanner, "ISBN:");
    ...
    public void removeBook(Scanner scanner) {
        ...
        String isbn = readIsbn(scanner, "Enter ISBN of book to remove");
        Book   book = isbnIndex.get(isbn);
    ...
    public void updateBook(Scanner scanner) {
        ...
        String isbn = readIsbn(scanner, "Enter ISBN of book to update");
        Book   book = isbnIndex.get(isbn);
        if (book == null) { printError("No book found with ISBN '" + isbn + "'."); return; }
    ...
    private void searchByIsbn(Scanner scanner) {
        String isbn = readIsbn(scanner, "Enter ISBN");
        Book   book = isbnIndex.get(isbn);
    ...
    public void borrowBook(Scanner scanner) {
        ...
        String isbn = readIsbn(scanner, "Enter ISBN of book to borrow");
        Book   book = isbnIndex.get(isbn);
    ...
    public void returnBook(Scanner scanner) {
        ...
        if (!borrowerIndex.containsKey(studentId)) {
            printError("No borrow record found for Student No. '" + studentId + "'.");
    ...
    public void markBookLost(Scanner scanner) {
        ...
        String isbn = readIsbn(scanner, "Enter ISBN of lost book (digits only)");
        Book   book = isbnIndex.get(isbn);
    ...
    private String readIsbn(Scanner scanner, String label) {
        while (true) {
            String input = prompt(scanner, label).trim();

            if (input.isEmpty()) {
                printError("ISBN cannot be blank. Enter digits only (e.g. 9780134685991).");
                continue;
            }
            ...
            if (!allDigits) {
                printError("Invalid ISBN '" + input + "' — ISBNs must contain digits only.");
                printError("Remove any hyphens, spaces, or letters. Try again (e.g. 9780134685991).");
                continue;
            }
